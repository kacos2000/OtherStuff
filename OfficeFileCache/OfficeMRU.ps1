...
$dir = "$($env:TEMP)\OfficeMRU"
if(!(Test-Path -Path $dir )){New-Item -ItemType directory -Path $dir
                             Write-Host "'$($dir)' created" -f yellow}
Write-Host "Saving MRUresults to (csv): 'OfficeMRU-$($snow).txt' in $dir`n" -f Yellow
$MRUresults|Export-Csv -Delimiter "|" -NoTypeInformation -Encoding UTF8 -Path "$($dir)\OfficeMRU-$($snow).txt" 
Invoke-Item $dir}
