...
# Save Console Output
Start-Transcript -path "$($env:TEMP)\Console Output - $($snow).txt" -append
...
$output|Export-Csv -Delimiter "|" -Encoding UTF8 -Path "$($env:TEMP)\FSF List - $($snow).txt" -NoTypeInformation
}
...
ForEach ($table in $tables)
{
    ...
    Write-Host "Saving Table 'MasterFile' to (csv): 'MasterFile - $($snow).txt' in $env:TEMP" -f Yellow
    $MasterFile| Export-Csv -Delimiter "|" -NoTypeInformation -Encoding UTF8 -Path "$($env:TEMP)\MasterFile - $($snow).txt"
    }
    else{
