...
if($OneDrive.count -ge1){$snow = Get-Date -Format FileDateTimeUniversal
$dir = "$($env:TEMP)\OneDrive"
if(!(Test-Path -Path $dir )){New-Item -ItemType directory -Path $dir
                             Write-Host "'$($dir)' created" -f yellow}
Write-Host "Saving 'OneDrive' Accounts list to (csv): 'OneDrive-$($snow).txt' in $($dir)`n" -f Yellow
$OneDrive|Export-Csv -Delimiter "|" -NoTypeInformation -Encoding UTF8 -Path "$($dir)\OneDrive-$($snow).txt"
Invoke-Item $dir
}
